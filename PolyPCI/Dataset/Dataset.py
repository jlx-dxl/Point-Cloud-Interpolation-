...
class NuscenesDataset(Dataset):
    def __init__(self, root, scenes_list, scene_split_lib='./Dataset/scene-split/', field=1,
                 npoints=int(16000), interval=5, if_random=False, random_times=1):
        # 文件目录相关
        ...
        # 模型参数相关
        self.npoints = npoints  # 读出时把点降采样到相同的个数，默认=16000
        self.interval = interval  # 插帧间隔默认=5
        ...
        # 组织数据集（元素为帧的名称）
        self.forward_frame_lists, self.key_frame_list, self.backward_frame_lists, self.t_list, self.T_list, self.gt_frame_list = self.make_dataset()
    ...
    def make_dataset(self):
        forward_frame_lists = []  # 用于储存前瞻点，元素：list，每个list长度：self.field
        backward_frame_lists = []  # 用于储存后瞻点，元素：list，每个list长度：self.field
        key_frame_list = []  # 用于储存关键帧（即相对时间的参考点），元素：frame_name
        T_list = []
        t_list = []  # 时间戳，元素：float
        ...
        for i in range(len(self.timestamp_list)):  # 遍历所有scene
            ...
            max_ind = len(times)
            key_frame_ind = self.field * self.interval

            while (key_frame_ind + self.field * self.interval < max_ind):

                if self.if_random == False:
                    for bias in range(- self.field * self.interval, self.field * self.interval + 1):
                        forw = []
                        backw = []
                        T = []

                        key_frame_list.append(fns[key_frame_ind])
                        T.append(0)
                        # print("key_frame_ind:", key_frame_ind)

                        for j in range(1, self.field + 1):
                            forw.append(fns[key_frame_ind - self.interval * j])
                            T.append((times[key_frame_ind - self.interval * j] - times[key_frame_ind]) / (
                                    times[key_frame_ind] - times[key_frame_ind - self.interval]))
                            # print("forward_frame_ind:", key_frame_ind - self.interval * j)
                            backw.append(fns[key_frame_ind + self.interval * j])
                            T.append((times[key_frame_ind + self.interval * j] - times[key_frame_ind]) / (
                                    times[key_frame_ind + self.interval] - times[key_frame_ind]))
                            # print("backward_frame_ind:", key_frame_ind + self.interval * j)
                        ...
                        if bias < 0:
                            t = (times[key_frame_ind + bias] - times[key_frame_ind]) / (
                                    times[key_frame_ind] - times[key_frame_ind - self.interval])
                        elif bias > 0:
                            t = (times[key_frame_ind + bias] - times[key_frame_ind]) / (
                                    times[key_frame_ind + self.interval] - times[key_frame_ind])
                        ...
                        gt_frame_list.append(fns[key_frame_ind + bias])
                        # print("gt_frame_ind:", key_frame_ind + bias)

                    key_frame_ind = key_frame_ind + self.interval

                else:
                    bias = np.random.randint(- self.field * self.interval, self.field * self.interval + 1,
                                             self.random_times)
                    print("bias:",bias)
                    forw = []
                    backw = []
                    ...
                    for k in range(self.random_times):
                        ...
                        if bias[k] < 0:
                            t = (times[key_frame_ind + bias[k]] - times[key_frame_ind]) / (
                                    times[key_frame_ind] - times[key_frame_ind - self.interval])
                        elif bias[k] > 0:
                            t = (times[key_frame_ind + bias[k]] - times[key_frame_ind]) / (
                                    times[key_frame_ind + self.interval] - times[key_frame_ind])
                        ...
                        gt_frame_list.append(fns[key_frame_ind + bias[k]])
                        print("gt_frame_ind:", key_frame_ind + bias[k])

                    key_frame_ind = key_frame_ind + self.interval

        return forward_frame_lists, key_frame_list, backward_frame_lists, t_list, T_list, gt_frame_list
    ...
    def __getitem__(self, index):
        ...
        T_list = np.asarray(self.T_list[index])

        key = self.get_lidar(os.path.join(self.root, self.key_frame_list[index]))
        key = torch.from_numpy(key).t().to(torch.float32)
        ...
        return forward_pcds, key, backward_pcds, t, T_list, gt, ini_feature
...
if __name__ == '__main__':
    ...
    dataset = NuscenesDataset(root='../../ISAPCI/Dataset/Subsets/Subset_01/LIDAR_TOP/',
                              ...
                              scene_split_lib='../../ISAPCI/Dataset/scene-split/', field=2, if_random=True,
                              random_times=1)
    # print(len(dataset.forward_frame_lists), len(dataset.key_frame_list), len(dataset.backward_frame_lists),
    ...
    #                             view_point_json_file="../Utils/ScreenCamera_2023-05-02-23-35-56.json", point_size=1.0)
    # for i, data in pbar:
