...
class PcdsVisualizer():
    def __init__(self, view_point_json_file, if_save=True, if_show=True, if_down_sample=True, npoints=12000,
    ...
    def read_bin_pc_fps_5(self, path, jitter=0):
        points = np.fromfile(path, dtype=np.float32).reshape(-1, 5)
        points = points[:, :3]  # open3d 只需xyz 与pcl不同
        points += np.random.normal(-jitter, jitter, size=points.shape)
...
if __name__ == '__main__':
    visualizer = PcdsVisualizer(if_save=True, if_show=False,view_point_json_file='ScreenCamera_2023-05-19-13-09-46.json',point_size=1.0)

    rootdir = './VIS/origin/'
    cat = os.listdir(rootdir)
    cat = cat[0:]
    # color = cycle([[1, 1, 0], [1, 0, 1], [0, 1, 1], [1, 0, 0]])
    for file in tqdm(cat):
        filename = os.path.join(rootdir, file)
        # print(filename)
        pcd = visualizer.read_bin_pc_fps_5(filename)  # 读取数据点
        # print('pcd found:', filename)
        visualizer.add_to_vis(pcd,[1,1,1])

        visualizer.show_and_save("../Demos/demo_0519/imgs/" + file + ".png")
        visualizer.clear()
