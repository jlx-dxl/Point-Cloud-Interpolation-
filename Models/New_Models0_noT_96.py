...
class ISAPCInet(nn.Module):
    def __init__(self, field, ff_out_c=96, tr_out_c=96, freeze=0):
    ...
    def forward(self, forward_pcds, key_pcds, backward_pcds, t, ini_feature, k=32):
        ...
        flow_forward_all = torch.cat(flow_forward_list, dim=-1)
        # print("flow_forward_all:", flow_forward_all.shape)  # [B,3,2*field*N]
        flow_backward_all = torch.cat(flow_backward_list, dim=-1)
        ...
        forwardff = self.ffab(flow_forward_all)
        # print("forwardffs_all:", forwardffs_all.shape)  # [B,ff_out_c,2*field*N]
        backwardff = self.ffab(flow_backward_all)
