...
class ISAPCInet(nn.Module):
    def __init__(self, field=0, ff_out_c=128, tr_out_c=128, freeze=0):
        ...
        self.outputer = Outputer(tr_out_c)
    ...
    def forward(self, forward_pcds, key_pcds, backward_pcds, t, ini_feature, k=32):
        ...
        flow_forward=self.flow(key_pcds[0], key_pcds[1], ini_feature, ini_feature)   # [B,3,N]
        flow_backward=self.flow(key_pcds[1], key_pcds[0], ini_feature, ini_feature)   # [B,3,N]
        
        
        forwardff = self.ffab(flow_forward)
        # print("forwardff:", forwardff.shape)  # [B,ff_out_c,2*field*N]
        backwardff = self.ffab(flow_backward)
        # print("backwardff:", backwardff.shape)  # [B,ff_out_c,2*field*N]

        forwardflow0, attn_forward = self.flow_tr_forward(flow_forward, forwardff)
        # print("forwardflow0:", forwardflow0.shape, "attn_forward:", attn_forward.shape)  # [B,tr_out_c,N]
        backwardflow0, attn_backward = self.flow_tr_backward(flow_backward, backwardff)
        # print("backwardflow0:", backwardflow0.shape, "attn_backward:", attn_backward.shape)  # [B,tr_out_c,N]

        forwardflow = self.outputer(forwardflow0)
        # print("forwardflow:", forwardflow.shape)  # [B,3,N]
        backwardflow = self.outputer(backwardflow0)
